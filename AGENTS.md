...
- **Walk-forward (樣本外驗證) goes through `lib.walk_forward.run_walk_forward`** — one call per validation, writing `strategies/<name>/wf.json` (the web 樣本外驗證 tab reads it). It counts as **one** iteration, never runs MCPT (that tab shows no p-value), and produces **no adoptable parameters** — do not edit `strategy.py` after one; adopting stays on the parameter-scan route. Never hand-roll a per-run scan loop. The web's third fixed prompt and the file contract: `references/lib.md` › *`lib/walk_forward.py`*.
