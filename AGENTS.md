...
- **Account library → snapshot is one atomic task:** Whenever you write or update any `lib/account_*.py` file (e.g. `lib/account_okx.py`), you MUST in the same session also update `manager/snapshot.py` to import from it and replace the `get_account_equity()` stub with real calls. `get_account_equity(exchange, env)` must return `{'equity': float, 'currency': str}`. API keys go in `.env` (same file as blave keys, e.g. `okx_api_key`, `okx_secret_key`, `okx_passphrase`). Before writing any `lib/account_*.py`, read the relevant skill reference under `skills/blave-quant/references/` for the correct balance endpoint.
