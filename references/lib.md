...
- `fetch_twstock_ohlcv(stock_id, schema, headers, start=None, end=None, adjust=False)` → Taiwan stock minute-line OHLCV DataFrame (Open/High/Low/Close/Volume); schema: `'1m'`/`'5m'`/`'15m'`/`'30m'`/`'60m'`/`'1d'`; **Volume in lots (張)**; bars are minute-start labels (13:30 Taipei bar = closing auction); `adjust=True` = forward-adjusted (後復權) OHLC for backtests across ex-dividend dates (volume unchanged; 503 fail-loud if factors unavailable); coverage is demand-driven — first query of a stock seeds only ~30 recent days and starts ongoing tracking, deep history (2019-01→) backfills server-side later, so check `fetch_twstock_ohlcv_symbols()` before deep backtests (see `references/twstock.md`)
