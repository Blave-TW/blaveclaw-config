...
- **Symbol contract:** every `get_positions()` returns `symbol` as canonical dashless uppercase (`BTCUSDT`, never the venue's `BTC-USDT` / `BTC_USDT`) — the reconciler keys on this format. Any strategy code comparing a position's symbol against a venue-format string must normalize BOTH sides first (`sym.replace('-', '').replace('_', '').upper()`); a raw comparison never matches and silently reads as "no position" (live incident: false position-closed notifications + strategy re-opening on top of an existing position). Spot inventory rows carry an `@spot` suffix on top of the canonical symbol — normalize the symbol part only (see the MARKET/spot keying section below)
