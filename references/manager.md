...
**A walk-forward is only as meaningful as the overlap.** If the members coexist for a small share of the union, most of the run is not testing the portfolio at all — the curve before the last member exists is one strategy's, and `managed_beats_pct_sharpe` is dominated by that stretch. Read the member spans before reading the verdict.
